...
function ProgramSwipe({ apiUrl, userId }) {
  ...
  const fetchFilteredProgram = async () => {
    try {
      ...
      while (
        processedPrograms.has(fetchedProgram.ProgramID) &&
      ...
      fetchedProgram.Duration =
        fetchedProgram.Duration === 1
          ? "half_day"
          : fetchedProgram.Duration === 2
          ? "whole_day"
          : fetchedProgram.Duration === 3
          ? "weekend"
          : fetchedProgram.Duration;
  ...
  const handleSwipe = async (action) => {
    ...
    try {
      console.log(
        `🔼 ${action.toUpperCase()} küldése: UserID = ${userId}, ProgramID = ${
          program.ProgramID
        }, RoomID = ${roomId || "Nincs"}`
      );
      const endpoint = action === "like" ? "like" : "dislike";
      const response = await axios.post(
        `${apiUrl}/programs/${program.ProgramID}/${endpoint}`,
        {
          userId,
  ...
  useEffect(() => {
    if (roomId) {
      axios
        .get(`${apiUrl}/rooms/${roomId}/users`, { withCredentials: true })
        .then((res) => {
  ...
  useEffect(() => {
    if (!isHost && roomId) {
      axios
        .get(`${apiUrl}/rooms/${roomId}/filters`, { withCredentials: true })
        .then((res) => {
          if (res.data) {
            setFilters(res.data);
  ...
  const handleEndSwipe = () => {
    if (roomId) {
      console.log(
        `🔄 Szobás válogatás vége, átirányítás a Summary oldalra. RoomID: ${roomId}`
      );
      navigate(`/summary?room=${roomId}`);
    } else {
      console.log(
        "🔄 Egyéni válogatás vége, átirányítás a LikedPrograms oldalra."
      );
    ...
    <div className="program-swipe-container">
      {(!roomId || isHost) && (
        <FilterComponent
          ...
          setFilters={(newFilters) => {
            setFilters(newFilters);
            if (roomId) {
              // Ha szobában van és host
              axios
                .post(
                  `${apiUrl}/rooms/${roomId}/filters`,
                  { filters: newFilters, userId },
                  { withCredentials: true }
                )
                .catch((err) =>
                  console.error("Hiba a szűrők mentése közben:", err)
                );
            }
          }}
      ...
      {!program && (
        <div className="program-card no-program-card">
          ...
          <h2>
            🎉 <span style={{ color: "#a855f7" }}>Gratulálunk!</span> 🎉
          </h2>
          <p>Minden elérhető programot végignéztél.</p>
          <p>
            🔄 Próbálj új keresést, vagy nézz vissza később új lehetőségekért!
          </p>
          <div className="end-buttons">
            <button className="reload-button" onClick={fetchFilteredProgram}>
              🔄 Újrapróbálkozás
            </button>
            <button onClick={handleEndSwipe} className="finish-button">
              🎯 Összegzés
            </button>
          </div>
        </div>
