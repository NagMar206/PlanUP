  ...
  const createRoom = async () => {
    try {
        const response = await axios.post(
            `${apiUrl}/rooms`,
            { userId, filters }, // Szűrők hozzáadása a kéréshez
            { withCredentials: true }
        );
...
const joinRoom = async () => {
    ...
    try {
        const response = await axios.post(
            `${apiUrl}/rooms/join`,
            { roomCode, userId, filters }, // Szűrők hozzáadása a kéréshez
            { withCredentials: true }
        );
  ...
  return (
    <div className="rooms-container">
      ...
      <p className="rooms-description">
        Hozz létre <span>szobát</span> barátaiddal, vagy <span>csatlakozz</span>{" "}
        meglévőhöz – és válogassatok együtt a legjobb programok közül!
      </p>
