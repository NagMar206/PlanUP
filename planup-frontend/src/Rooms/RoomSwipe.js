...
function RoomSwipe({ apiUrl }) {
    ...
    const handleSwipe = async (liked) => {
        ...
        try {
            if (liked) {
                await axios.post(`${apiUrl}/programs/${currentProgram.ProgramID}/like`, { userId: activeUserId, roomCode }, { withCredentials: true });
            } else {
    ...
    return (
        <div className="program-swipe-container">
            {isHost && (
                <FilterComponent
                    ...
                    setFilters={(newFilters) => {
                        setFilters(newFilters);
                        setFilterActive(true);
                        axios.post(`${apiUrl}/rooms/${roomCode}/filters`, { filters: newFilters, userId }, { withCredentials: true });
                    }}
            ...
            <div className="program-card">
                ...
                <p>⏳ {program.Duration === 1 ? "Fél napos" : program.Duration === 2 ? "Egész napos" : "Hétvégés"}</p>
            </div>
